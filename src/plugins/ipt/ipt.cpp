...
int annotate_ipt(drakvuf_t drakvuf, ipt* plugin, unsigned int vcpu_id, uint32_t* payloads, size_t num_payloads)
{
    ...
    uint8_t* buf = plugin->vcpu[vcpu_id].buf;
    uint64_t size = plugin->vcpu[vcpu_id].size;
    FILE* fd = plugin->vcpu[vcpu_id].fd;
    ...
    : pluginex(drakvuf, output)
{
    ...
    {
        if (!drakvuf_enable_ipt(drakvuf, i, &this->vcpu[i].buf, &this->vcpu[i].size))
        {
        ...
        this->vcpu[i].fd = fopen(ss.str().c_str(), "wb");
